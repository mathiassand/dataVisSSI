...
#   mutate(
#     custlng_Ch1 = X + (abs(custlng_dcr7dPer100kCh1)),
#     custlng_Ch3 = X + (abs(custlng_dcr7dPer100kCh3)),
#     custlng_Ch7 = X + (abs(custlng_dcr7dPer100kCh7)),
#     custlat_Ch1 = Y + (abs(custlat_dcr7dPer100kCh1)),
#     custlat_Ch3 = Y + (abs(custlat_dcr7dPer100kCh3)),
#     custlat_Ch7 = Y + (abs(custlat_dcr7dPer100kCh7)),
#   )
...
#also tried with dots over two lines per kommune didn't work
# long<- a_one_date %>% 
#   select(kommune,X,custlng_Ch1) %>%
#   pivot_longer(cols=!(kommune),names_to="longType",values_to = "long") %>% select (-longType)
# lat<-a_one_date %>% 
#   select(kommune,Y,custlat_Ch1) %>%
#   pivot_longer(cols=!(kommune),names_to="longType",values_to = "lat") %>% select (-longType)
# ll <- cbind(lat,long=long$long)
# a_one_date<-merge(ll,a_one_date)

    for(i in 1:nrow(a_one_date)){
      myMap = myMap %>%
        # addCircleMarkers(lng= ~X, lat = ~Y, radius = .3,data = a_one_date[i,])%>%
        addPolylines(data = a_one_date[i,],
                     lng= ~ c(X, custlng_Ch1),
                     lat= ~ c(Y, custlat_Ch1),
...
myMap
  ...
  #   lapply(list(a_one_date$kommune[a_one_date$kommune=="Aalborg"]), function(x) {
  #           addPolylines(myMap, 
  #                        data = a_one_date[a_one_date$kommune == x, ], 
  #                        lng = ~long, 
  #                        lat = ~lat,
  #                        color = 'red',
  #                         weight = 2) })
  #   
  # 
  #   addPolylines(data=a_one_date[a_one_date$kommune %in% c("Aalborg"),], 
  #   lng=~c(X, custlng_Ch1), lat=~c(Y, custlat_Ch1),
  #                    color = 'red',
  #                    weight = 2)
  # 
