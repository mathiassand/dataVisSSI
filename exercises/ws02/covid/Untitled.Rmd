...
# merging the covid data into the dataset with the coords - apparently this can't be plotted (or takes an eternity), might too much data in the frame
...
change_icons <- iconList(
  ch1_icon = makeIcon(ifelse(one_date$casesDPer100k < 0.2, "rect_green.png", "rect_red.png"), iconWidth = one_date$dcr7dPer100kCh1 + 20, iconHeight = "10"),
  ch3_icon = makeIcon(ifelse(one_date$casesDPer100k < 0.2, "rect_green.png", "rect_red.png"), iconWidth = one_date$dcr7dPer100kCh3 + 20, iconHeight = "10"),
  ch7_icon = makeIcon(ifelse(one_date$casesDPer100k < 0.2, "rect_green.png", "rect_red.png"), iconWidth = one_date$dcr7dPer100kCh7 + 20, iconHeight = "10")
) # values in dcr7dperk100kch_1/3/7 are too small to get plottet - idk how to normalize
...
leaflet() %>%
  ...
  # addRectangles(
  # data = one_date, ~X, ~Y, ~X, ~Y,
  # color="red",
  # opacity = 1.0,
  # weight = one_date$PosTestRate7d * 100,
  # popup = ~ as.character(one_date$PosTestRate7d),
  # label = ~ as.character(one_date$kommune)
  addMarkers(
    data = one_date, ~X, ~Y,
    icon = ch1_icon
  ) %>%
