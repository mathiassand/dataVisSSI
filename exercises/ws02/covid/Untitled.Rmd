...
leaflet() %>%
  ...
  addPolygons(
    data = df_dk, color = "#444444", weight = 1, smoothFactor = 0.5,
    opacity = 1.0, fillOpacity = 0.5,
    fillColor = ~ colorQuantile("YlOrRd", population)(population),
    label = dk_merge_coords$kommune
  ) %>%
  addMarkers(data = dk_merge_coords, ~X, ~Y, popup = ~ as.character(population), label = ~ as.character(kommune)) 
