...
leaflet() %>% addTiles() %>% 
  ...
  addPolygons(data = df_dk, color = "#444444", weight = 1, smoothFactor = 0.5,
    ...
    label = df_dk$kommune) %>% addCircleMarkers(data=df_dk, lng=9.98229, lat=57.46417, popup = ~as.character(population), label = ~as.character(kommune)) #at the moment shows hjørrings population - it's because it's the last entry in the data.frame
